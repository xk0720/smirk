...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_path', type=str, default='samples/test_image1.png',
    ...
    if args.use_smirk_generator:
        ...
        checkpoint_generator = {k.replace('smirk_generator.', ''): v for k, v in checkpoint.items() if
                                'smirk_generator' in k}  # checkpoint includes both smirk_encoder and smirk_generator
    ...
    if args.render_orig:
        if args.crop:
            ...
            rendered_img_orig = F.interpolate(rendered_img, (orig_image_height, orig_image_width),
                                              mode='bilinear').cpu()
        ...
        if args.render_orig:
            if args.crop:
                reconstructed_img_numpy = (
                            reconstructed_img.squeeze(0).permute(1, 2, 0).detach().cpu().numpy() * 255.0).astype(
                    np.uint8)
                reconstructed_img_orig = warp(reconstructed_img_numpy, tform,
                                              output_shape=(orig_image_height, orig_image_width),
                                              preserve_range=True).astype(np.uint8)
                # back to pytorch to concatenate with full_image
                reconstructed_img_orig = torch.Tensor(reconstructed_img_orig).permute(2, 0, 1).unsqueeze(
                    0).float() / 255.0
            else:
                reconstructed_img_orig = F.interpolate(reconstructed_img, (orig_image_height, orig_image_width),
                                                       mode='bilinear').cpu()
